...
#### 游玩须知：
1. **《现实化诺文斯克》只有类似《诺文斯克潜行者》的生存难度，并使用Path to Tarkov来替代Traveler实现类无缝地图，少了部分功能。**
2. **《现实化诺文斯克》v0.9.0版SAIN预设-NorvinskStalkerAIpreset**还在不断调整期，可以对战局内AI行为和难度提出建议。
3. **《现实化诺文斯克》v0.9.x版好奇猫自用游戏设置及按键**默认开启，如果你有自己的游玩设置，请关掉或自己修改。
...
- 淘汰
	- 无
- 与正式版的距离
	- [《生活在诺文斯克》Live in Norvinskv开发进度](《生活在诺文斯克》Live%20in%20Norvinskv开发进度.md)
...
# 25-01-05-v0.9.8.0更新日志：

### ⚠️提醒：0980更新必须删除整个MO2文件夹即可，可以不重开档，但如要切换AI生成MOD：SWAG和MOAR的话，必须重开档。
### 特别鸣谢：感谢@Kang @小火山Volcano及小火山群里的哔哔波哔波制作的汉化程序，没有他们就无法实现全MOD汉化。
### 特别声明：本次更新中包含的我翻译的内容均使用Deepseek AI进行批量翻译，如有翻译错误，请告知。

### 更新内容
- 新增：
	- [6]AI动态生成点-SWAG+Donuts回归，可作为398时并不成熟的MOAR（虽然截至250105，也只是堪堪能用）的替换。但SWAG已经停更，虽然有人接手，但启动艰难。
	- 汉化分类——[14]汉化模组，增加：
		- [14]ConfigurationManager客户端模组翻译器（3.x-4.x通用） 及已翻译文本。已包含本整合包的所有客户端MOD（F12）的选项的翻译。
		- [14]服务器端MOD文本通用汉化补丁-Simple Translator 文本汉化for3.9.8，可通过统一格式的汉化补丁对所有服务器端MOD物品、武器、配件及任务进行汉化。
		- 极个别MOD的零碎汉化。
- 更新：
	- 将《现实化诺文斯克》MOD商人任务文本翻译调整到[14]汉化模组分类，改为[14]MOD商人任务文本翻译，包含整合包所有mod任务线的汉化。
- 调整：
	- [6]超精简AI生成-DewardianDev-MOAR，调整AI生成数量和频率。
	- 再次调整Order.json，改进MOD间的兼容性。
