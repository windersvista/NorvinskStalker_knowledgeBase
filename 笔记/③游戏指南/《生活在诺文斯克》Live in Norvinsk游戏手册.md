...
> [!NOTE]
> #### 整合包信息：
> 《生活在诺文斯克》Live in Norvinsk整合包是基于SPT3.11.4制作。
> 作为《诺文斯克潜行者》整合包1.1.8之后的全新重制的离线塔科夫整合包。
> 
> #### **特色简介**：
> 
> 在原版基础上通过150+Mod，特别是核心MOD：PathToTarkov（PTT）进行地图间移动设置、CustomProfiles（自定义档案）增加专属起始角色、Realism（现实主义）进行更进一步现实化改造（《诺潜》已经有一定现实化改造了），具备撬锁、枪破门、技能大改、医疗方式大修、AI行为拟人化、武器装备的数值重新平衡等等，提供区别于原版塔科夫的真实性沉浸式游戏体验。

### 整合包游玩重要Q&A
- 请熟练使用F12
- 一些无法修复的又不太影响游戏的已知小BUG：
	- 整合包中大部分食水都能堆叠，但吃喝堆叠的食水会出现食水增加值不显示的bug。**避免方式：拆开单个再吃喝即可避免。**
	- 由于[5]武器操控大修-TarkovIRL_W.H.M对固定武器的兼容性比较差，从榴弹炮或者机枪下来后，会导致武器失控乱晃。**修复方式：下来后瞄准状态开两枪即可，或关掉武器操控大修。**
	- 负重不建议过低，比如-100kg，因为可能会有手臂乱舞，甚至身手分离的问题出现。这是现实主义MOD改变了游戏负重逻辑的原因。
	- 整合包游玩时出现的1%low帧跳帧主要源于AI系统。因为塔科夫的AI系统缺陷，除非使用第二台电脑（即使是虚拟机上的）跑服务器，否则无法避免AI系统对CPU的残害（）。
...
整合包主要使用以上五个核心MOD的不同设置来实现四种难度，整合包默认是最难的生存难度，所以请开始游戏前选择自己的难度：
	...
	各个难度的差异明细如下：
	![](assets/Pasted%20image%2020260210215417.png)

#### 2.不同开局也各有不同
整合包新增的四（五）种不同的开局角色，可以在新建档案的列表中找到它们：
![](assets/Pasted%20image%2020260210215408.png)
...
《生活在诺文斯克》整合包是通过[6]Acid的AI生成系统-botplacementsystem来管理AI刷新。因为对这个MOD设置进行了调整，设置文件放在了《生活在诺文斯克》AI生成-botplacementsystem沉浸感增强设置中。
玩家如果要对AI数量进行调整，请直接双击下图中的程序，不用通过MO2执行：
![](assets/Pasted%20image%2020260206011111.png)

然后在程序中按自己的想法设置：
![](assets/Pasted%20image%2020260206011232.png)
