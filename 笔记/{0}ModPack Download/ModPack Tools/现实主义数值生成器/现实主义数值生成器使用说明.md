...
## 0. 今日更新（2026-03-11 / v3.11）

- 瞄具识别增强：扩展 `ModType=sight` 的名称关键词，新增 `1p87`、`comp_m4/compm4`、`boss_xe` 及历史拼写兼容（如 `aimpooint`）。
- `ScopeTemplates` 兜底顺序优化：名称关键词未命中时优先回落 `scope_red_dot`，降低通用 `parent` 回填导致的高倍率误判。
- 武器二级修正收敛：`stockless`（无抵肩）后坐增量区间下调，减少冲锋枪极端后坐离群值。
- `mount` 档位策略对齐：规则不再为导轨/镜座注入无语义的 `HeatFactor/CoolFactor` 占位值。
